...
int initialization( struct sockaddr ** internet_address, socklen_t * internet_address_length )
{
	...
	int getaddrinfo_return = getaddrinfo( "192.168.1.57", "24042", &internet_address_setup, &internet_address_result );
...
void execution(int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	...
	number_of_bytes_send = sendto( internet_socket, msg , 2, 0, internet_address, internet_address_length );
...
{
	int aantal = 0; 
	printf("geef het aantal packetten dat u wilt ontvangen: \n");
	scanf("%d",&aantal); 
